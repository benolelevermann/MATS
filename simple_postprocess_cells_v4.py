...
def outward_direction(
    ...
    vector = np.asarray(endpoint, dtype=float) - np.asarray(path[-1], dtype=float)
...
def angle_degrees(
    ...
    connection = np.asarray(target, dtype=float) - np.asarray(start, dtype=float)
...
def connection_geometry(
    ...
    """
    Berechnet die Geometrie einer möglichen Verbindung.
    ...
    Rückgabe:
        ...
        forward_distance:
            Strecke in Vorwärtsrichtung des Endpunkts.
            Negative Werte bedeuten, dass das Ziel hinter dem Endpunkt liegt.

        lateral_offset:
            Seitlicher Abstand des Ziels von der verlängerten
            Skeletonrichtung in Pixeln.
    """
    ...
    if distance < 1e-6:
        return 180.0, 0.0, float("inf")

    connection_direction = delta / distance

    cosine = float(
        np.clip(
            np.dot(direction, connection_direction),
            -1.0,
            1.0,
        )
    )
    ...
    # Projektion des Zielvektors auf die Skeletonrichtung.
    forward_distance = float(np.dot(direction, delta))

    lateral_squared = max(
        distance**2 - forward_distance**2,
        0.0,
    )

    lateral_offset = math.sqrt(lateral_squared)

    return angle, forward_distance, lateral_offset
...
def soma_contacts(
    ...
    for soma_id, object_slice in enumerate(
        ...
        local_soma = soma_labels[y0:y1, x0:x1] == soma_id
        ...
        local_components = skeleton_labels[y0:y1, x0:x1]
...
def estimate_component_soma_owners(
    ...
    tree = cKDTree(boundary_points.astype(np.float64))
    ...
    for component_id, component_slice in enumerate(
        ...
        if actual_contacts is not None:
            contacts = actual_contacts.get(component_id, set())

            if len(contacts) == 1:
                owners[component_id] = int(next(iter(contacts)))
                continue
        ...
        points = np.argwhere(component_labels[component_slice] == component_id)
        ...
        for point in points:
            ...
            for distance, boundary_index in zip(distances, indices):
                ...
                soma_id = int(boundary_soma_ids[boundary_index])

                if soma_id == 0:
                    continue

                distance = float(distance)
                old_distance = best_distance_per_soma.get(soma_id)

                if old_distance is None or distance < old_distance:
                    best_distance_per_soma[soma_id] = distance
        ...
        if second_distance - best_distance < owner_margin:
            owners[component_id] = -1
        else:
            owners[component_id] = int(best_soma_id)
...
class Components:
    ...
    def __init__(
        ...
        self.somas = {
            component_id: set(contacts.get(component_id, set()))
            for component_id in range(1, count + 1)
        }

        self.owner = {
            component_id: int(provisional_owners.get(component_id, 0))
            for component_id in range(1, count + 1)
        }

    def root(self, component_id: int) -> int:
    ...
    def soma_ids(self, component_id: int) -> set[int]:
        return self.somas[self.root(component_id)]

    def provisional_owner(self, component_id: int) -> int:
        return int(self.owner[self.root(component_id)])

    def merge_allowed(self, first: int, second: int) -> bool:
        ...
        first_owner = int(self.owner[root_first])
        second_owner = int(self.owner[root_second])
        ...
        if combined_somas:
            actual_soma = next(iter(combined_somas))

            if first_owner > 0 and first_owner != actual_soma:
                return False

            if second_owner > 0 and second_owner != actual_soma:
                return False
    ...
    def assign_soma(
        ...
        provisional_owner = int(self.owner[root])

        if provisional_owner == -1:
            return False

        if provisional_owner > 0 and provisional_owner != soma_id:
            return False
...
def draw_connection_along_skeleton(
    ...
    ymin = max(0, min(y0, y1) - corridor_pad)
    ymax = min(
        reference_skeleton.shape[0],
        max(y0, y1) + corridor_pad + 1,
    )

    xmin = max(0, min(x0, x1) - corridor_pad)
    xmax = min(
    ...
    distance_to_skeleton = ndi.distance_transform_edt(~local_reference)
    ...
    cost[distance_to_skeleton <= 2.0] *= 0.35

    if soma_labels is not None and allowed_soma_id is not None:
        local_soma_labels = soma_labels[
            ymin:ymax,
            xmin:xmax,
        ]

        forbidden = (local_soma_labels != 0) & (local_soma_labels != allowed_soma_id)
    ...
    local_start = (y0 - ymin, x0 - xmin)
    local_target = (y1 - ymin, x1 - xmin)
    ...
    try:
        ...
        rr = np.asarray(
            [point[0] + ymin for point in path],
            dtype=np.int32,
        )

        cc = np.asarray(
            [point[1] + xmin for point in path],
            dtype=np.int32,
        )

    except Exception:
        # Fallback, falls die lokale Pfadsuche fehlschlägt.
        rr, cc = line_pixels(start, target)

    added_connections[rr, cc] = True

    if connection_width > 1:
        radius = max(1, connection_width // 2)

        yy0 = max(0, int(rr.min()) - radius)
        yy1 = min(
            added_connections.shape[0],
            int(rr.max()) + radius + 1,
        )

        xx0 = max(0, int(cc.min()) - radius)
        xx1 = min(
...
def connect_gaps(
    ...
    if len(soma_boundary_points) > 0:
        soma_tree: Optional[cKDTree] = cKDTree(soma_boundary_points.astype(np.float64))
    else:
        soma_tree = None
    ...
    for round_number in range(1, rounds + 1):
        ...
        points = [tuple(int(value) for value in point) for point in endpoint_array]
        ...
        provisional_owners = estimate_component_soma_owners(
        ...
        endpoint_tree = cKDTree(endpoint_array.astype(np.float64))
        ...
        for first_index, second_index in endpoint_pairs:
            ...
            crossed_components = np.unique(component_labels[rr, cc])
            ...
            distance = float(
                np.linalg.norm(
                    np.asarray(first_point, dtype=float)
                    - np.asarray(second_point, dtype=float)
                )
            )
        ...
        if soma_tree is not None:
            for endpoint_index, endpoint in enumerate(points):
                ...
                for boundary_index in nearby_boundary_indices:
                    ...
                    rr, cc = line_pixels(endpoint, target)

                    crossed_somas = np.unique(soma_labels[rr, cc])

                    crosses_foreign_soma = any(
                        int(value) != soma_id for value in crossed_somas if value != 0
                    )
        ...
        for candidate in soma_candidates:
            ...
            if not groups.assign_soma(component_id, soma_id):
                continue
    ...
    return added_connections, report
...
def rescue_unassigned_components(
    ...
    rescue_lateral_tolerance: float,
    ...
    for round_number in range(1, rescue_rounds + 1):
        ...
        for component_id in range(1, component_count + 1):
            ...
            if len(contacted_somas) == 1:
                soma_id = int(next(iter(contacted_somas)))
                component_to_soma[component_id] = soma_id
        ...
        if not anchored_component_ids or not free_component_ids:
            break
        ...
        number_of_neighbors = min(64, len(target_points))
        ...
        for free_component_id in free_component_ids:
            ...
            for endpoint_index in endpoint_indices:
                endpoint = (
                    int(endpoint_array[endpoint_index, 0]),
                    int(endpoint_array[endpoint_index, 1]),
                )
                ...
                for distance, target_index in zip(
                    ...
                    target = (
                        int(target_points[target_index, 0]),
                        int(target_points[target_index, 1]),
                    )
                    ...
                    if (
                        ...
                        and target_soma_id != free_component_owner
                    ):
                        continue
                    ...
                    if direction is None:
                        # Bei sehr kleinen oder verzweigten Fragmenten
                        # lässt sich die Richtung nicht zuverlässig bestimmen.
                        ...
                        # Ziele hinter dem Endpunkt grundsätzlich ablehnen.
                        if forward_distance <= 0:
                            continue

                        # Verbindung akzeptieren, wenn entweder:
                        # 1. der Winkel ausreichend gut ist oder
                        # 2. der seitliche Versatz klein genug ist.
                        if (
                            current_angle > max_rescue_angle
                            and lateral_offset > rescue_lateral_tolerance
                        ):
                            continue
                    ...
                    crossed_soma_ids = np.unique(soma_labels[rr, cc])

                    if any(int(value) != 0 for value in crossed_soma_ids):
                        continue

                    # Keine bereits anders zugeordnete
                    # Skeletonkomponente kreuzen.
                    crossed_component_ids = np.unique(component_labels[rr, cc])
                    ...
                    score = (
                        distance
                        + 0.20 * float(current_angle)
                        + 0.75 * float(lateral_offset)
                    )

                    # Sehr kleine Komponenten leicht benachteiligen.
                    if int(component_sizes[free_component_id]) < 5:
                        score += 5.0
                    ...
                    old_candidate = best_candidate_per_soma.get(target_soma_id)
            ...
            candidates.append(sorted_candidates[0])
        ...
        for candidate in candidates:
            ...
            if not np.any(~current_skeleton[rr, cc]):
                continue
        ...
        print("  Rescue-Verbindungen akzeptiert: " f"{accepted_this_round}")

        if accepted_this_round == 0:
            break

    return rescue_connections, report
...
def keep_connected_pairs(
    ...
    return kept_skeleton, kept_soma
...
def save_qc(
    ...
    raw_small = raw_segmentation[::step, ::step]
    final_small = final_segmentation[::step, ::step]
    added_small = added_connections[::step, ::step]
    removed_skeleton_small = removed_skeleton[::step, ::step]
    removed_soma_small = removed_soma[::step, ::step]
    ...
    if original is None:
        ...
        base = normalize(original[::step, ::step])

    raw_rgb = np.repeat(base[..., None], 3, axis=2) * 0.4
    raw_rgb[raw_small == 1] = [0.0, 1.0, 1.0]
    raw_rgb[raw_small == 2] = [1.0, 1.0, 1.0]

    final_rgb = np.repeat(base[..., None], 3, axis=2) * 0.4
    final_rgb[final_small == 1] = [0.0, 1.0, 0.0]
    final_rgb[final_small == 2] = [1.0, 0.0, 1.0]

    changes_rgb = np.repeat(base[..., None], 3, axis=2) * 0.3
...
def main() -> None:
    ...
    parser.add_argument(
        "--rescue-lateral-tolerance",
        type=float,
        default=10,
        help=(
            "Maximal erlaubter seitlicher Versatz eines Rescue-Ziels "
            "von der verlängerten Endpunktrichtung in Pixeln."
        ),
    )
    ...
    skeleton_after_strict = raw_skeleton | strict_connections
    ...
    rescue_connections, rescue_report = rescue_unassigned_components(
        ...
        rescue_lateral_tolerance=(args.rescue_lateral_tolerance),
    ...
    unresolved_skeleton = completed_skeleton & ~final_skeleton
    ...
    segmentation_before_cleanup[completed_skeleton] = 1
    segmentation_before_cleanup[raw_soma] = 2
    ...
    tifffile.imwrite(
        output_dir / "05_unresolved_skeleton.tif",
        unresolved_skeleton.astype(np.uint8),
        photometric="minisblack",
    )

    tifffile.imwrite(
        output_dir / "06_segmentation_before_strict_cleanup.tif",
        segmentation_before_cleanup,
        photometric="minisblack",
    )

    tifffile.imwrite(
        output_dir / "07_final_segmentation_strict.tif",
        final_segmentation,
        photometric="minisblack",
    )
