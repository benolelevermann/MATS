...
if ((Test-Path -LiteralPath $FinalCheckpoint -PathType Leaf) -and -not $Continue) {
    throw "This R5 run is already complete: $FinalCheckpoint"
}
if ((Test-Path -LiteralPath $FoldFolder -PathType Container) -and -not $Continue) {
    ...
    if ($NonLogFiles.Count -gt 0) {
        throw "An incomplete R5 result folder already exists. Use -Continue to resume it: $FoldFolder"
    }
    if ($ExistingFiles.Count -gt 0) {
        Write-Host "A previous R5 start stopped before epoch 0. Keeping its log and retrying safely."
    }
}
if ($Continue -and -not (
...
)) { throw "No R5 checkpoint is available to continue: $FoldFolder" }

$Mode = if ($FullRun) { "1000-epoch full run" } else { "50-epoch technical test" }
Write-Host "Experiment R5: label-safe R0 plus DeepFlux auxiliary learning"
Write-Host "Mode: $Mode"
Write-Host "Trainer: $Trainer"
Write-Host "Flux radius/weight: 7 px / 0.1"
Write-Host "Inference output: unchanged semantic 0/1/2 head"
...
& $Python @Arguments
if ($LASTEXITCODE -ne 0) { throw "R5 training failed with exit code $LASTEXITCODE" }
...
Write-Host "R5 finished. Validation TIFF/NPZ: $TiffCount/$NpzCount"
