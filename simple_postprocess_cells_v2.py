...
def nearest_soma_for_component(
    component_id: int,
    component_labels: np.ndarray,
    soma_boundary_points: np.ndarray,
    soma_labels: np.ndarray,
    k: int = 64,
) -> tuple[Optional[int], dict[int, float]]:
    """
    Liefert das nächstgelegene Soma zu einer freien Skeletonkomponente
    sowie minimale Distanzen zu gefundenen Soma-Kandidaten.
    """
    points = np.argwhere(component_labels == component_id)
    if len(points) == 0 or len(soma_boundary_points) == 0:
        return None, {}

    # Nicht alle Punkte nehmen, sonst wird es unnötig langsam
    if len(points) > 50:
        idx = np.linspace(0, len(points) - 1, 50).astype(int)
        points = points[idx]

    tree = cKDTree(soma_boundary_points)
    soma_best: dict[int, float] = {}

    for point in points:
        distances, indices = tree.query(
            point.astype(np.float64),
            k=min(k, len(soma_boundary_points)),
        )
        ...
        for dist, ind in zip(distances, indices):
            soma_id = int(
                soma_labels[
                    soma_boundary_points[ind, 0],
                    soma_boundary_points[ind, 1],
                ]
            )
            if soma_id == 0:
                continue
            ...
            dist = float(dist)
            old = soma_best.get(soma_id)
            if old is None or dist < old:
                soma_best[soma_id] = dist

    if not soma_best:
        return None, {}

    best_soma = min(soma_best, key=soma_best.get)
    return best_soma, soma_best
...
def draw_connection_along_skeleton(
    ...
    target_soma_id: Optional[int] = None,
    soma_labels: Optional[np.ndarray] = None,
    corridor_pad: int = 8,
) -> None:
    """
    Verbindet start und target nicht als gerade Linie,
    sondern über einen kürzesten Pfad, der vorhandenes Skeleton bevorzugt.
    """
    y0, x0 = start
    y1, x1 = target

    ymin = max(0, min(y0, y1) - corridor_pad)
    ymax = min(reference_skeleton.shape[0], max(y0, y1) + corridor_pad + 1)
    xmin = max(0, min(x0, x1) - corridor_pad)
    xmax = min(reference_skeleton.shape[1], max(x0, x1) + corridor_pad + 1)

    local_ref = reference_skeleton[ymin:ymax, xmin:xmax]
    local_soma = soma_mask[ymin:ymax, xmin:xmax]

    # Distanz zum vorhandenen Skeleton
    dist_to_skel = ndi.distance_transform_edt(~local_ref)

    # Grundkosten:
    # auf Skeleton billig, nahe dran moderat, weit weg teurer
    cost = 1.0 + np.clip(dist_to_skel, 0, 6) * 1.5
    cost[local_ref] = 0.05
    cost[dist_to_skel <= 2] *= 0.35

    # Soma vermeiden, außer ggf. Ziel-Soma
    if soma_labels is not None and target_soma_id is not None:
        local_soma_labels = soma_labels[ymin:ymax, xmin:xmax]
        forbidden = (local_soma_labels != 0) & (local_soma_labels != target_soma_id)
        cost[forbidden] = 1e6
    else:
        cost[local_soma] = 1e6

    start_local = (y0 - ymin, x0 - xmin)
    target_local = (y1 - ymin, x1 - xmin)
    ...
    rr = np.asarray([p[0] + ymin for p in path], dtype=np.int32)
    cc = np.asarray([p[1] + xmin for p in path], dtype=np.int32)
    ...
    if connection_width > 1:
        ...
        yy1 = min(added_connections.shape[0], int(rr.max()) + radius + 1)
        xx0 = max(0, int(cc.min()) - radius)
        xx1 = min(added_connections.shape[1], int(cc.max()) + radius + 1)

        local = added_connections[yy0:yy1, xx0:xx1]
        added_connections[yy0:yy1, xx0:xx1] = ndi.binary_dilation(
            local,
            iterations=radius,
        )
...
def rescue_unassigned_components(
    ...
    for round_number in range(
        ...
        for free_component_id in free_component_ids:
            ...
            nearest_soma_id, soma_distance_map = nearest_soma_for_component(
                component_id=free_component_id,
                component_labels=component_labels,
                soma_boundary_points=np.argwhere(
                    soma_mask & ~ndi.binary_erosion(soma_mask)
                ),
                soma_labels=soma_labels,
            )
            ...
            for endpoint_index in endpoint_indices:
                ...
                for distance, target_index in zip(
                    ...
                    target_soma_id = int(target_soma_ids[target_index])
                                        # Wenn ein freies Fragment klar näher an einem bestimmten
                    # Soma liegt, dann nur dieses Soma zulassen.
                    if nearest_soma_id is not None and soma_distance_map:
                        best_dist = soma_distance_map[nearest_soma_id]
                        cand_dist = soma_distance_map.get(target_soma_id, np.inf)

                        # Wenn das Ziel-Soma deutlich schlechter ist, verwerfen
                        if cand_dist > best_dist + 12:
                            continue
                    ...
                    # Bonus/Penalty je nachdem, wie gut das Ziel-Soma
                    # zur freien Komponente passt
                    if nearest_soma_id is not None and soma_distance_map:
                        best_dist = soma_distance_map[nearest_soma_id]
                        cand_soma_dist = soma_distance_map.get(target_soma_id, best_dist + 20)
                        score += 1.5 * (cand_soma_dist - best_dist)
        ...
        for candidate in candidates:
            ...
            draw_connection_along_skeleton(
                ...
                soma_mask=soma_mask,
                connection_width=connection_width,
                target_soma_id=int(candidate["target_soma"]),
                soma_labels=soma_labels,
            )
